/*
Program Name: 05_calculate_power.sas
Program Creator: Matthew Psioda;

This SAS program is designed to complete stage two of the sample size determination simulations. That is, 
this program is designed to compute the Bayesian power for given combinations of the number 
of events targeted in the new trial and optimal values of a0. The program simulates new trial datasets based on 
input design settings, analyzes the data using the partial-borrowing power prior, and then computes 
estimates of the Bayesian power using the results from the simulated trials. 
...
[4] &root./data/simulation_results/calculate_power --> location to which simulation analysis results are written (RESULTS SAS library);
...
the controls.simulation_controls_alt dataset created by a previously run program.

This program includes the macro definition for the %LOOP macro that reads in the simulation setting 
observations from the controls.simulation_controls_alt dataset that are assigned to the compute node 
in question and then loops over the simulation settings to perform the specified number of simulations 
for each simulation setting (i.e., observation in the controls.simulation_controls_alt dataset).
...
data work.__controls__;
 set controls.simulation_controls_alt;
...
proc sort data = work.__controls__; 
 by category nsp asp shareblh nuTarget alpha_targ a0 inner_idx; 
run;
...
%if %sysfunc(mod(&loop,100))=1 %then %put loop &loop of &nSimSettings.;
  ...
  data _null_;
   ...
   call symput('dsSamples',strip(asp));
  ...
  ** collapse data dynamically (treatment only borrowing);
...
data results.calc_power_&dsLabel.;
 ...
 by category nsp asp shareblh nuTarget alpha_targ a0;
  ...
  keep category nsp asp shareblh nuTarget alpha_targ a0 N rejRate;
run;
...
%*in general sysparm is designed to be set from the command line so that this program
  ...
  can always set sysparm themselves or directly set node+id;

%let node_id = %scan("&sysparm", 1, " ");
%put &=node_id;

** indicator for whether to use debug mode. If debug mode is on then
   at most three design parameter settigs will be evaluated and 
